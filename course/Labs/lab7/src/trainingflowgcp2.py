...
class TrainingFlowGCP(FlowSpec):
    """
    Enhanced training flow for GCP deployment with Kubernetes support.
    ...
    @kubernetes(cpu=2, memory=4000)  # Request 2 CPU cores and 4GB memory
    @retry(times=3)  # Retry up to 3 times on failure
    @timeout(minutes=30)  # Set 30-minute timeout
    @catch(var='train_error')  # Catch any errors for handling
    @step
